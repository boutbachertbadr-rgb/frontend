...
export default function CheckoutModal() {
  ...
  const onSubmit = (data: CheckoutForm) => {
    ...
    if (available.length === 0) {
      ...
      const addrParam = encodeURIComponent(JSON.stringify({ name: data.name, phone: data.phone, state: data.state, city: data.city, distrito: data.distrito, address: data.address, reference: data.reference }));
      router.push(`/thank-you?order_id=${orderId}&total=${total.toFixed(2)}&upsell=false&addr=${addrParam}&items=${itemsParam}`);
      clearCart();
      createOrder({
