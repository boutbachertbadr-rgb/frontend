...
export default function UpsellOverlay() {
  ...
  function handleContinue(didAccept: boolean) {
    ...
    const addrParam = encodeURIComponent(JSON.stringify({ name: customerForm?.name ?? "", phone: customerForm?.phone ?? "", state: customerForm?.state ?? "", city: customerForm?.city ?? "", distrito: customerForm?.distrito ?? "", address: customerForm?.address ?? "", reference: customerForm?.reference ?? "" }));

    router.push(`/thank-you?order_id=${orderId}&total=${total.toFixed(2)}&upsell=${hasUpsell}&addr=${addrParam}&items=${itemsParam}`);
    ...
    createOrder({
      ...
      customer_distrito: customerForm?.distrito ?? "",

      customer_address: customerForm?.address ?? "",

      customer_reference: customerForm?.reference ?? "",
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-400 text-xs line-through">
                          ...
                          ₡{upsell.originalPrice.toLocaleString("es-CR")} CRC
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <p className="text-brand font-bold text-base">
                            ...
                            ₡{upsell.discountPrice.toLocaleString("es-CR")} CRC

                          </p>

                          <span className="bg-accent/10 text-accent text-[10px] font-bold px-1.5 py-0.5 rounded-full">

                            -₡{(upsell.originalPrice - upsell.discountPrice).toLocaleString("es-CR")}
              ...
              {selectedCount > 0 && (
                ...
                <div className="text-center">
                  ...
                  <p className="text-brand font-bold text-lg">
                    ...
                    Ahorras ₡{totalSavings.toLocaleString("es-CR")} CRC
