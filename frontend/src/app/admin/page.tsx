...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem("vzl_admin_token") ?? "";
    const savedRole = (sessionStorage.getItem("vzl_admin_role") ?? "admin") as "admin" | "operator";
  ...
  async function api(path: string, opts?: RequestInit) {
    ...
    if (res.status === 403 && !isOperator) { setToken(""); sessionStorage.removeItem("vzl_admin_token"); sessionStorage.removeItem("vzl_admin_role"); throw new Error("Session expired. Please login again."); }
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { sessionStorage.removeItem("vzl_admin_token"); sessionStorage.removeItem("vzl_admin_role"); setToken(""); setRole("admin"); }} className="text-xs text-gray-400 hover:text-red-500 px-2 py-1 transition">Logout</button>
          </div>
        </div>
      </header>
