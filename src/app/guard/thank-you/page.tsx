...
function GuardThankYouContent() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12 font-body" style={{ backgroundColor: "#0A0A0A" }}>
      <div className="w-full max-w-md space-y-5">

        <div className="flex items-center justify-center gap-2.5">
          <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: "#FF4500" }}>
            <span className="font-heading font-bold text-base leading-none text-white">V</span>
          </div>
          <div className="flex flex-col items-start leading-none">
            <span className="font-heading font-bold text-lg tracking-tight text-white">VAZLINA</span>
            <span className="text-[10px] font-medium mt-0.5 tracking-widest uppercase text-gray-400">Tecnología curada</span>
          </div>
        </div>

        <div className="rounded-3xl p-6 text-center" style={{ backgroundColor: "#111111" }}>
          <CheckCircle size={44} className="mx-auto mb-3" style={{ color: "#FF4500" }} />
          <h1 className="text-2xl font-bold text-white mb-1">¡Pedido Confirmado!</h1>
          <p className="text-gray-400 text-sm">Pedido #{orderId}</p>
          <div className="mt-4 inline-block px-5 py-2 rounded-full text-sm font-bold" style={{ backgroundColor: "rgba(255,69,0,0.15)", color: "#FF4500" }}>
            Total a pagar al recibir: ₡{total} CRC
          </div>
          <p className="text-gray-500 text-xs mt-3 flex items-center justify-center gap-1">
            <ShieldCheck size={12} style={{ color: "#FF4500" }} /> Nuestro equipo te contactará lo antes posible para confirmar tu pedido
          </p>
        </div>
        ...
        {items.length > 0 && (
          <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: "#111111" }}>
            ...
            <div className="px-5 py-3 flex items-center justify-between" style={{ borderTop: "1px solid #2a2a2a", backgroundColor: "rgba(255,69,0,0.08)" }}>
        ...
        {addr && (
          <div className="rounded-2xl p-5" style={{ backgroundColor: "#111111" }}>
            <p className="font-semibold text-sm mb-3 text-white">Dirección de entrega</p>
            <div className="text-xs text-gray-500 space-y-1 leading-relaxed">
              <p><strong className="text-gray-300">{addr.name}</strong> · {addr.phone}</p>
              <p>{addr.address}</p>
              <p>{addr.distrito}, {addr.city}, {addr.state}</p>
              {addr.reference && <p className="text-gray-400">Ref: {addr.reference}</p>}
            </div>
            <p className="text-xs mt-4 pt-3" style={{ borderTop: "1px solid #2a2a2a", color: "#FF4500" }}>
        ...
        <div className="rounded-2xl p-5" style={{ backgroundColor: "#111111" }}>
          <div className="flex gap-3 items-start mb-4">
            ...
            <div>
              <p className="font-semibold text-sm mb-1 text-white">{isExpress ? "Envío Express · Pago al Recibir" : "Envío Gratis · Pago al Recibir"}</p>
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              <p className="font-semibold text-sm mb-1 text-white">Importante: Atendé nuestra llamada</p>
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap text-xs text-gray-500">
          <span className="flex items-center gap-1"><ShieldCheck size={13} style={{ color: "#FF4500" }} /> Garantía 30 días</span>
